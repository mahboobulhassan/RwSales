...
def data_prep(df):
    ...
    return dfa
def Data(conn):
    query = text("SELECT Date_,Description,QTY,Value,CP FROM Trans_Details Where Date_>='01/01/2023' AND Date_<='12/31/2025' ")
    pdf=pd.read_sql_query(query,con=conn)
    df=data_prep(pdf)
    return df
def max_data(conn):
    query = text("SELECT Date_,Description,Supplier_Name,Department,Category,OP,QTY,Value,CP FROM Trans_Details Where Date_>='01/01/2023' AND Date_<='12/31/2025'")
    pdf=pd.read_sql_query(query,con=conn)
    df=data_prep(pdf)
    return df
