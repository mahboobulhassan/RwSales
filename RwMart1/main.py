...
def main():
    ...
    df=st.sidebar.file_uploader('Select File to Upload')
    st.markdown("### :blue[Upload data file]")
    st.write('Data file should contain columns Date_, Description, Department, Category, OP, Supplier_Name, CP, QTY, Value')
       
    if df:
        
        dfa=pd.read_csv(df)
        cols=dfa.columns
        list=['Date_', 'Description','Department','Category','OP','Supplier_Name','CP','QTY','Value']
        missingCol=[]
        for col in cols:
            if col not in list:
                missingCol.append(col)
        if missingCol:
            st.write('Missing columns in data: ',missingCol)
        else:
            data=data_prep(dfa)                
            if martEda:
                maxEda=False
                ...
                if selected_product:
                    ...
                    if currentYear==previousYear:
                        st.markdown(f"<h4 style='color:green;'>Sale During {previousYear}</h4>",unsafe_allow_html=True)
                    else:                  
                        st.markdown(f"<h4 style='color:green;'>Sale During {previousYear} & {currentYear}</h4>",unsafe_allow_html=True)
